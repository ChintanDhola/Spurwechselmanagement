...
def plot_results(**kwargs):
    if os.path.isfile('results/cut_out/FSM_cut_out.csv'):
        ...
        dfIDM = pd.read_csv('results/cut_out/IDM_MOBIL_cut_out.csv')
    else:
    ...
    for ur in range(10, 131, 10):
        plt.figure(str(ur) + ' IDM_MOBIL')
        plt.title('IDM_MOBIL')
        df = dfIDM[dfIDM['velocity'] == ur]

        idm_mask1 = (df['Crash']) & (df['Crash_front'] != True)
        idm_mask2 = (~df['Crash']) & (~df['Crash_front'])
        idm_mask3 = df['Crash_front']

        plt.plot(df[idm_mask1]['front_distance'], -df[idm_mask1]['lateral_velocities'], 'rx', markersize=10, alpha=0.2,
                 label='Crash')
        plt.plot(df[idm_mask2]['front_distance'], -df[idm_mask2]['lateral_velocities'], 'go', markersize=10, alpha=0.2,
                 label='Safe')
        plt.plot(df[idm_mask3]['front_distance'], -df[idm_mask3]['lateral_velocities'], 'yo', markersize=10, alpha=0.2,
                 label='Crash leader')
        ...
        plt.tight_layout()
        if kwargs.get('save_image', False):
            plt.savefig('results/cut_out/IDM_MOBIL_' + str(ur) + '.png')
            plt.close()
        else:
            plt.show()