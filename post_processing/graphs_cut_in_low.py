...
def plot_results(**kwargs):
    ...
    for ego_speed in [10, 20, 30, 40, 50, 60]:
        for cut_in_speed in range(10, ego_speed, 10):
            ...
            df_Fuz = pd.read_csv(f'results/cut_in_low_speed/FSM_{ego_speed}_{cut_in_speed}.csv')
            df_CC = pd.read_csv(f'results/cut_in_low_speed/CC_human_driver_{ego_speed}_{cut_in_speed}.csv')
            df_IDM = pd.read_csv(f'results/cut_in_low_speed/IDM_MOBIL_{ego_speed}_{cut_in_speed}.csv')
            ...
            plt.plot(df_IDM['long_dist'], df_IDM['lat_vel'], 'cs', markersize=10, alpha=0.5, label='IDM + MOBIL')
            ...
            if kwargs.get('save_image', False):
                plt.savefig(f'results/cut_in_low_speed/comparison_for_speeds_{ego_speed}_{cut_in_speed}.png', dpi=150)
                plt.close()
            else:
                plt.show()
