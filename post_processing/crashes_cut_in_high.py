...
def plot_results():
    ...
    for ego_speed in [70, 90, 110, 130]:
        for cut_in_speed in range(10, ego_speed, 30):
            ...
            df_FSM = pd.read_csv(f'results/cut_in_high_speed/FSM_{ego_speed}_{cut_in_speed}.csv')
            df_CC = pd.read_csv(f'results/cut_in_high_speed/CC_human_driver_{ego_speed}_{cut_in_speed}.csv')
            df_IDM = pd.read_csv(f'results/cut_in_high_speed/IDM_MOBIL_{ego_speed}_{cut_in_speed}.csv')

            RSS[0] = RSS[0] + len(df_RSS)
            Reg157[0] = Reg157[0] + len(df_Reg157)
            FSM[0] = FSM[0] + len(df_FSM)
            CC[0] = CC[0] + len(df_CC)
            IDM[0] = IDM[0] + len(df_IDM)

            df_RSS = df_RSS[df_RSS['Crash']]
            df_Reg157 = df_Reg157[df_Reg157['Crash']]
            df_FSM = df_FSM[df_FSM['Crash']]
            df_CC = df_CC[df_CC['Crash']]
            df_IDM = df_IDM[df_IDM['Crash']]

            RSS[1] = RSS[1] + len(df_RSS)
            Reg157[1] = Reg157[1] + len(df_Reg157)
            FSM[1] = FSM[1] + len(df_FSM)
            CC[1] = CC[1] + len(df_CC)
            IDM[1] = IDM[1] + len(df_IDM)

    print("Crash percentage RSS high speed:    ", RSS[1] / RSS[0] * 100)
    print("Crash percentage Reg157 high speed: ", Reg157[1] / Reg157[0] * 100)
    print("Crash percentage FSM high speed:    ", FSM[1] / FSM[0] * 100)
    print("Crash percentage CC high speed:     ", CC[1] / CC[0] * 100)
    print("Crash percentage IDM_MOBIL:        ", IDM[1] / IDM[0] * 100)
