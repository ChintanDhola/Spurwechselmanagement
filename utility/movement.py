...
def control(ego_veh, cutting_in_veh, freq, model_check, model_react, i):
    ...
    # --- IDM CONTINUOUS CONTROL LOGIC ---
    if ego_veh.use_idm:
        # 1. Determine Leader
        # In this simulation, the "cutting_in_veh" is the only other object.
        # We treat it as the leader if it is ahead of us.
        if cutting_in_veh.pos_profile_long[i] > ego_veh.pos_profile_long[i]:
            ...
            d_lead = 1000.0  # Virtual infinite distance
            v_lead = 0.0

        # 2. Calculate IDM Acceleration
        acc = idm_mobil.calculate_idm_accel(speed_log, v_lead, d_lead, ego_veh)

        # 3. Calculate New Speed (Longitudinal)
        new_speed_long = max(0, speed_log + acc / freq)

        # 4. Lateral Speed (Simple pass-through, IDM doesn't steer)
        new_speed_lat = speed_lat

        # 5. Run Safety Check (MOBIL) purely for statistics/logging
        # This decides if the point is marked "Safe" (Green) or "Unsafe" (Red) on plots
