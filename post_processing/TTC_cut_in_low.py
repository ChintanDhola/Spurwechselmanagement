...
def plot_results(**kwargs):
    ...
    if not os.path.isfile('results/cut_in_low_speed/'+use_model+'_20_10.csv'):
    ...
    for ego_speed in [10, 20, 30, 40, 50, 60]:
        for cut_in_speed in range(10, ego_speed, 10):
            ...
            if use_model == 'FSM':
                ...
                df = pd.read_csv(f'results/cut_in_low_speed/CC_human_driver_{ego_speed}_{cut_in_speed}.csv')
            elif use_model == 'RSS':
                df = pd.read_csv(f'results/cut_in_low_speed/RSS_{ego_speed}_{cut_in_speed}.csv')
            elif use_model == 'Reg157':
                df = pd.read_csv(f'results/cut_in_low_speed/Reg157_{ego_speed}_{cut_in_speed}.csv')
            elif use_model == 'IDM_MOBIL':
                df = pd.read_csv(f'results/cut_in_low_speed/IDM_MOBIL_{ego_speed}_{cut_in_speed}.csv')
            else:
            ...
            if kwargs.get('save_image', False):
                plt.savefig(f'results/cut_in_low_speed/TTC_for_speeds_{ego_speed}_{cut_in_speed}_{namefig}', dpi=300)
                plt.close()
            else:
                plt.show()
