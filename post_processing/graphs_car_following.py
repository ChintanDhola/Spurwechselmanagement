...
def plot_results(**kwargs):
    if os.path.isfile('results/car_following/FSM_car_following.csv'):
        ...
        dfIDM = pd.read_csv('results/car_following/IDM_MOBIL_car_following.csv')
    else:
    ...
    # --- ADD THIS BLOCK ---
    plt.figure('IDM_MOBIL')
    plt.title('IDM + MOBIL')
    # Filter crashes
    idm_crash_mask = dfIDM['Crash'] == True
    plt.plot(dfIDM[idm_crash_mask]['velocity'], dfIDM[idm_crash_mask]['max_deceleration'], 'rx', markersize=10,
            alpha=0.2, label='Crash')
    plt.plot(dfIDM[~idm_crash_mask]['velocity'], dfIDM[~idm_crash_mask]['max_deceleration'], 'go',
            markersize=10, alpha=0.2, label='Safe')
    plt.ylabel('Deceleration rate (g)')
    plt.xlabel('Ego vehicle velocity (km/h)')
    plt.tight_layout()
    if kwargs.get('save_image', False):
        plt.savefig('results/car_following/IDM_MOBIL_CF.png', dpi=150)
        plt.close()
    # ----------------------
    ...
    df = dfIDM[np.abs(dfIDM['max_deceleration'] - 0.05) < 0.01]
    plt.plot(df['velocity'], df['initial_distance'] / df['velocity'] * 3.6, label='IDM THW')
